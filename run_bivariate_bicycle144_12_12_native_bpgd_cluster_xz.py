#!/usr/bin/env python3
"""Headless cluster runner for 144_12_12 native BPGD with X-basis filtering.

This matches the older `xyz=False` / `XZ` decoding style by filtering the
memory-X circuits to detectors sensitive to X-basis data errors before building
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for csv outputs and plots/. Defaults to examples/notebook_data/bicycle_bivariate_144_12_12_native_bpgd_cluster_xz under the repo root.",
    )
    parser.add_argument(
        "--reset-data",
        action="store_true",
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else repo_root / "examples" / "notebook_data" / "bicycle_bivariate_144_12_12_native_bpgd_cluster_xz"
    )
    plots_dir = output_dir / "plots"
    output_dir.mkdir(parents=True, exist_ok=True)
    plots_dir.mkdir(parents=True, exist_ok=True)

    save_stem = "bicycle_bivariate_144_12_12_native_bpgd_cluster_xz"
    ...
    for step_index, decoder_name in selected_decoder_steps:
        ...
        step_plot_path = plots_dir / f"XZ_step_{step_index:02d}_after_{decoder_name}.png"
        plot_summary(
            summary_df,
            decoder_sequence=decoder_sequence,
            title=f"Cumulative logical error rate after {decoder_name} (XZ filtered)",
            save_path=step_plot_path,
        )

    final_plot_path = plots_dir / "XZ_combined_all_decoders.png"
    plot_summary(
        summary_df,
        decoder_sequence=decoder_sequence,
        title="Logical error rate: all decoders (XZ filtered)",
