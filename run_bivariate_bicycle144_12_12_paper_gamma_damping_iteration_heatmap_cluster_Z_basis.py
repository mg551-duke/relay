#!/usr/bin/env python3
"""Detailed cluster runner for paper-gamma Relay-BP plus random damping heatmaps.

This mirrors the gross-code detailed damping heatmap workflow, but keeps the
paper memory schedule active: the first leg uses a fixed `gamma0 = 0.125`,
then later Relay legs sample gamma weights from the paper interval while
per-leg edge damping intervals are swept across a center/width grid.

Defaults:
- circuit: `bicycle_bivariate_144_12_12_memory_Z`
- XZ-style decoding via `Z`-basis detector filtering
- p = 3e-3
- gamma0 = 0.125 on the first leg, then relay gammas sampled from [-0.24, 0.66]
...
from run_bivariate_bicycle144_12_12_paper_iteration_heatmap_cluster_Z_basis import (
    attach_iteration_metrics,
    best_points_table,
    build_task,
    find_circuit_path,
    find_repo_root,
...
PAPER_RELAY_GAMMA_LOW = -0.24
PAPER_RELAY_GAMMA_HIGH = 0.66
...
def build_setup_dict(
    *,
    ...
    return {
        ...
        "explicit_gamma_mode": "sample_after_first_leg",
        "relay_gamma_interval_low": float(PAPER_RELAY_GAMMA_LOW),
        "relay_gamma_interval_high": float(PAPER_RELAY_GAMMA_HIGH),
...
def validate_resume_setup(setup_json_path: Path, current_setup: dict[str, Any]) -> None:
    ...
    fields_to_compare = [
        ...
        "base_relay_seed",
        "explicit_gamma_mode",
        "relay_gamma_interval_low",
        "relay_gamma_interval_high",
        "reference_interval_low",
        "reference_interval_high",
        "damping_interval_handling",
    ]
...
def plot_damping_heatmap(
    *,
    ...
    ax.set_title(
        "Relay-BP + random damping logical error rate | "
    ...
    subtitle = (
        ...
        f"relay gammas ~ U[{PAPER_RELAY_GAMMA_LOW:.2f}, {PAPER_RELAY_GAMMA_HIGH:.2f}], relay_posteriors = {relay_posteriors}, "
        + (
...
def plot_collection_heatmaps(
    *,
    ...
    for index, (ax, (title, data, cmap, norm)) in enumerate(
        zip(flat_axes, specs, strict=False)
    ):
    ...
    fig.suptitle(
        "Relay-BP + random damping collection summary heatmaps | "
        f"{metadata.get('circuit', 'unknown')} | p = {selected_p:.3g} | basis filter = {basis_filter}"
        + (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "examples/notebook_data/bicycle_bivariate_144_12_12_paper_gamma_damping_iteration_heatmap_cluster_Z_basis "
            "under the repo root."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    metadata["memory_disabled"] = False
    metadata["gamma_schedule"] = "sample_after_first_leg"
    ...
    print(
        f"relay gamma interval after first leg = "
        f"[{PAPER_RELAY_GAMMA_LOW:.3f}, {PAPER_RELAY_GAMMA_HIGH:.3f}]"
    )
