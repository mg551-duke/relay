#!/usr/bin/env python3
"""Grid search the second-previous term for Bernoulli relay message mix."""
...
SECOND_PREVIOUS_PARAMETER_NAMES = [
    "second_previous_negative",
    "second_previous_positive",
    "second_previous_p_positive",
]
PARAMETER_NAMES = [*BASE_PARAMETER_NAMES, *SECOND_PREVIOUS_PARAMETER_NAMES]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--message-mix-params-path", type=Path, required=True)
    ...
    parser.add_argument(
        "--second-previous-negative-values", type=str, default="-0.5,-0.25,-0.1,0.0"
    )
    parser.add_argument(
        "--second-previous-positive-values", type=str, default="0.0,0.1,0.25,0.5"
    )
    parser.add_argument(
        "--second-previous-probability-values",
        type=str,
...
def main() -> None:
    ...
    base_params, base_source = _load_best_base_params(args.message_mix_params_path)
    ...
    grid = _parameter_grid(args, base_params)
    candidates = [
    ...
    _write_json(
        args.output_dir / "grid_search_config.json",
        _config_payload(
            args=args,
    ...
    *,
    ...
    for repeat_idx in range(args.distribution_repeats):
        ...
        decoder = relay_bp.RelayedBPGDDecoderF64(
            ...
            message_mix_bernoulli=_base_params_tuple(params),
            message_mix_second_previous_bernoulli=_second_previous_params_tuple(params),
...
def _parameter_grid(
    ...
    values = [
        _parse_float_values(
            args.second_previous_negative_values, "second-previous-negative-values"
        ),
        _parse_float_values(
            args.second_previous_positive_values, "second-previous-positive-values"
        ),
        _parse_float_values(
            args.second_previous_probability_values,
            "second-previous-probability-values",
        ),
    ]
    grid = []
...
def _load_best_base_params(path: Path) -> tuple[dict[str, float], Path]:
    if path.is_file():
        payload = _load_json(path)
        return _extract_base_params(payload), path
    if not path.exists():
        raise FileNotFoundError(f"message-mix params path does not exist: {path}")
    direct = path / "best_params.json"
    if direct.exists():
        payload = _load_json(direct)
        return _extract_base_params(payload), direct

    candidates = sorted(path.glob("seed_*/best_params.json"))
    if not candidates:
        raise FileNotFoundError(
            f"No best_params.json files found under message-mix params path: {path}"
        )
    ranked = sorted(
        ((_rank_payload(_load_json(candidate)), candidate) for candidate in candidates),
        key=lambda item: item[0],
    )
    best_path = ranked[0][1]
    return _extract_base_params(_load_json(best_path)), best_path


def _extract_base_params(payload: dict[str, Any]) -> dict[str, float]:
    if "best_candidate" in payload and isinstance(payload["best_candidate"], dict):
        payload = payload["best_candidate"]
    params = payload.get("params", payload)
    if "message_mix_bernoulli" in params:
        values = params["message_mix_bernoulli"]
        params = dict(zip(BASE_PARAMETER_NAMES, values, strict=True))
    missing = [name for name in BASE_PARAMETER_NAMES if name not in params]
    if missing:
        raise ValueError(f"Missing base message-mix params: {missing}")
    return {name: float(params[name]) for name in BASE_PARAMETER_NAMES}


def _rank_payload(payload: dict[str, Any]) -> tuple[float, float, float, str]:
    if "best_candidate" in payload and isinstance(payload["best_candidate"], dict):
        payload = payload["best_candidate"]
...
def _validate_params(params: dict[str, float]) -> None:
    for name in ("fresh_negative", "previous_negative", "second_previous_negative"):
        if params[name] > 0.0:
            raise ValueError(f"{name} must be <= 0.")
    for name in ("fresh_positive", "previous_positive", "second_previous_positive"):
        if params[name] < 0.0:
            raise ValueError(f"{name} must be >= 0.")
    for name in (
        "fresh_p_positive",
        ...
        if not 0.0 <= params[name] <= 1.0:
            raise ValueError(f"{name} must be in [0, 1].")
    if not all(np.isfinite(list(params.values()))):
        raise ValueError(f"Message-mix params must be finite: {params}")


def _base_params_tuple(
    params: dict[str, float],
) -> tuple[float, float, float, float, float, float]:
    return tuple(float(params[name]) for name in BASE_PARAMETER_NAMES)  # type: ignore[return-value]


def _second_previous_params_tuple(
    params: dict[str, float],
) -> tuple[float, float, float]:
    return tuple(float(params[name]) for name in SECOND_PREVIOUS_PARAMETER_NAMES)  # type: ignore[return-value]
...
def _select_circuit(args: argparse.Namespace) -> Path:
    ...
    if len(paths) != 1:
        raise SystemExit(
            "Second-previous message-mix grid search expects exactly one circuit "
            f"per run. Matched {len(paths)} circuits; use --filename-contains."
        )
    return paths[0]
...
def _decomposed_hyperedges_arg(args: argparse.Namespace) -> bool | None:
    if args.decomposed_hyperedges and args.allow_undecomposed_hyperedges:
        raise SystemExit(
            "Use at most one of --decomposed-hyperedges and "
            "--allow-undecomposed-hyperedges."
        )
...
def _config_payload(
    *,
    ...
    payload["message_mix_params_path"] = str(args.message_mix_params_path)
    payload["selected_message_mix_params"] = base_params
    payload["selected_message_mix_params_source"] = str(base_source)
...
def _best_payload(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "message_mix_bernoulli": [
            float(row[name]) for name in BASE_PARAMETER_NAMES
        ],
        "message_mix_second_previous_bernoulli": [
            float(row[name]) for name in SECOND_PREVIOUS_PARAMETER_NAMES
        ],
        "metrics": {
            "shots": int(row["shots"]),
