#!/usr/bin/env python3
"""Train Bernoulli relay message-mix weights on a fixed Stim circuit sample."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fresh-negative-min", type=float, default=-1.0)
    parser.add_argument("--fresh-negative-max", type=float, default=0.0)
    parser.add_argument("--fresh-positive-min", type=float, default=0.0)
    parser.add_argument("--fresh-positive-max", type=float, default=1.0)
    parser.add_argument("--fresh-probability-min", type=float, default=0.001)
    parser.add_argument("--fresh-probability-max", type=float, default=0.999)
    parser.add_argument("--previous-negative-min", type=float, default=-1.0)
    parser.add_argument("--previous-negative-max", type=float, default=0.0)
    parser.add_argument("--previous-positive-min", type=float, default=0.0)
    parser.add_argument("--previous-positive-max", type=float, default=1.0)
    parser.add_argument("--previous-probability-min", type=float, default=0.001)
    parser.add_argument("--previous-probability-max", type=float, default=0.999)
...
def main() -> None:
    ...
    def progress_callback(progress: dict[str, Any]) -> None:
        ...
        _write_generation_csv(
            args.output_dir / "generation_metrics.csv", progress["generation_records"]
        )
        _write_json(
            checkpoint_path,
            {
    ...
    result = relay_bp.train_relayed_bpgd_bernoulli_message_mix(
        ...
        r_high=args.r_high,
        t_high=args.t_high,
        fresh_negative_min=args.fresh_negative_min,
        fresh_negative_max=args.fresh_negative_max,
        fresh_positive_min=args.fresh_positive_min,
        fresh_positive_max=args.fresh_positive_max,
        fresh_probability_min=args.fresh_probability_min,
        fresh_probability_max=args.fresh_probability_max,
        previous_negative_min=args.previous_negative_min,
        previous_negative_max=args.previous_negative_max,
        previous_positive_min=args.previous_positive_min,
        previous_positive_max=args.previous_positive_max,
        previous_probability_min=args.previous_probability_min,
        previous_probability_max=args.previous_probability_max,
        ...
        validation_max_logical_failures=_positive_or_none(
            args.validation_max_logical_failures
        ),
        seed=args.seed,
    ...
    _write_generation_csv(
        args.output_dir / "generation_metrics.csv", result["generation_records"]
    )
    _write_json(best_params_path, result["best_candidate"])
    _write_json(
        args.output_dir / "validation_metrics.json", result["validation_metrics"]
    )
    _write_json(
        checkpoint_path,
        {
...
def _select_circuit(args: argparse.Namespace) -> Path:
    ...
    if len(paths) != 1:
        raise SystemExit(
            "Bernoulli message-mix training expects exactly one circuit per run. "
...
def _config_payload(
    args: argparse.Namespace, circuit_path: Path, basis_filter: str
) -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "best_fresh_negative": best_params["fresh_negative"],
                "best_fresh_positive": best_params["fresh_positive"],
                "best_fresh_p_positive": best_params["fresh_p_positive"],
                "best_previous_negative": best_params["previous_negative"],
                "best_previous_positive": best_params["previous_positive"],
                "best_previous_p_positive": best_params["previous_p_positive"],
                ...
                "mean_fresh_negative": mean_params["fresh_negative"],
                "mean_fresh_positive": mean_params["fresh_positive"],
                "mean_fresh_p_positive": mean_params["fresh_p_positive"],
                "mean_previous_negative": mean_params["previous_negative"],
                "mean_previous_positive": mean_params["previous_positive"],
                "mean_previous_p_positive": mean_params["previous_p_positive"],
                "raw_std_fresh_negative": record["raw_std"][0],
                "raw_std_fresh_positive": record["raw_std"][1],
                "raw_std_fresh_probability": record["raw_std"][2],
                "raw_std_previous_negative": record["raw_std"][3],
                "raw_std_previous_positive": record["raw_std"][4],
                "raw_std_previous_probability": record["raw_std"][5],
            }
        )
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=list(rows[0].keys()) if rows else ["generation"]
        )
        writer.writeheader()
