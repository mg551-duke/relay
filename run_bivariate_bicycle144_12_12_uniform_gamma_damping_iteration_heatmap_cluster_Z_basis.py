#!/usr/bin/env python3
"""Detailed cluster runner for uniform-gamma Relay-BP plus random damping heatmaps.

This mirrors the gross-code detailed damping heatmap workflow, but keeps the
memory effect active at a uniform value on every Relay leg while sweeping
per-leg edge damping intervals across a center/width grid.

Defaults:
- circuit: `bicycle_bivariate_144_12_12_memory_Z`
- XZ-style decoding via `Z`-basis detector filtering
- p = 3e-3
- gamma0 = 0.125 on the first leg and explicit relay gammas = 0.125 everywhere
...
def build_setup_dict(
    *,
    ...
    return {
        ...
        "explicit_gamma_mode": "uniform_all_legs",
        "uniform_relay_gamma": float(UNIFORM_RELAY_GAMMA),
...
def plot_damping_heatmap(
    *,
    ...
    subtitle = (
        ...
        f"relay gammas fixed at {UNIFORM_RELAY_GAMMA:.3f}, relay_posteriors = {relay_posteriors}, "
        + (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "examples/notebook_data/bicycle_bivariate_144_12_12_uniform_gamma_damping_iteration_heatmap_cluster_Z_basis "
            "under the repo root."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    save_stem = "bicycle_bivariate_144_12_12_uniform_gamma_damping_iteration_heatmap_cluster_Z_basis"
    output_dir = (
    ...
    metadata["gamma_schedule"] = "uniform_all_legs"
    ...
    print(f"explicit relay gammas = {UNIFORM_RELAY_GAMMA:.3f} everywhere")
